...
#    pragma pack(pop)
    ...
    typedef NTSTATUS(WINAPI* NtQueryInformationFile_t)(
        HANDLE hFile, PIO_STATUS_BLOCK IoStatusBlock, PVOID FileInformation, ULONG Length, NT_FILE_INFORMATION_CLASS FileInformationClass);
            ...
            if (INVALID_HANDLE_VALUE != hFile)
            {
                ...
                do
                {
                    ...
                    status = m_NtQueryInformationFile(
                        hFile, &iostat, reinterpret_cast<PVOID>(m_Buffer.getBuf()), static_cast<ULONG>(m_Buffer.getByteCount()), FileInformationStream);
